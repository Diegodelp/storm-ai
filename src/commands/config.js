 ...
 * @param {string} [input.stack]              Free-text stack label (legacy + 'other').
 * @param {string} [input.stackId]            Preset id from src/core/stacks.js.
 * @param {string} [input.database]
 * @param {string} [input.databaseId]
 * @param {{provider:string,name:string|null}} [input.model]
 * @param {string} [input.agent]              CLI agent id ('claude-code', 'opencode', ...).
 * @param {{customCommand?: string}} [input.launch]  Optional launch overrides.
...
function validateAndNormalize(raw) {
  ...
  config.description ??= '';
  config.stack ??= '';
  config.stackId ??= 'other';
  config.database ??= '';
  config.databaseId ??= 'other';

  config.agent ??= 'claude-code';
  if (typeof config.agent !== 'string') {
    throw new ConfigError('`agent` must be a string.');
  }

  config.launch ??= {};
  if (typeof config.launch !== 'object') {
    throw new ConfigError('`launch` must be an object.');
  }
  if (config.launch.customCommand !== undefined &&
      typeof config.launch.customCommand !== 'string') {
...
function normalizeCompactContext(cc) {
  ...
  cc.branches = cc.branches.map((b, i) => {
    ...
    if (b.stale === true) out.stale = true;
    return out;
  });
