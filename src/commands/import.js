 ...
 * @property {{provider: string, model?: string|null}|null} [analysis]
 *   Provider used to classify every function into sections (the function
 *   index). null/omitted = path-based classification only.
 * @property {(done: number, total: number) => void} [onFunctionProgress]
...
export async function writeImport(plan) {
  ...
  try {
    ...
    const classifier = plan.analysis?.provider
      ? makeLlmClassifier({
        provider: plan.analysis.provider,
        model: plan.analysis.model ?? null,
        language: config.compact_context.function_language ?? 'es',
        onProgress: plan.onFunctionProgress,
      })
      : null;
    const refreshed = await refreshCompactContext(plan.projectRoot, {
    ...
    result.functions = refreshed.functions;
    for (const w of refreshed.warnings.filter((x) => x.includes('clasificar funciones'))) {
      result.warnings.push(w);
    }
    if (!classifier && refreshed.functions.pending > 0) {
      result.warnings.push(
        `${refreshed.functions.pending} función(es) clasificadas solo por ruta (sin IA). ` +
          'Corré `storm refresh` para que la IA las ordene por sección.',
      );
    }
  } catch (err) {
...
export async function runImportNonInteractive(input) {
  ...
  const result = await writeImport({
    ...
    model: launch,
    agent,
    analysis: input.skipLLM ? null : { provider, model },
    onFunctionProgress: (done, total) => log(`  funciones clasificadas: ${done}/${total}`),
    branches,
    skills,
    agents,
    ...overrides,
  ...
  if (result.functions) {
    log(`✓ ${result.functions.total} función(es) indexadas en ${result.functions.sections} sección(es) ` +
        `(.context-compact/sections/).`);
  }
